...
{
    /// <summary>
    /// Backend database communication for the Nova Service
    /// This handles the actual HTTP communication with your Rust backend
    /// Only the service should use this - products communicate through NovaCore
    /// </summary>
    public class DatabaseBackend : IDisposable
    {
        private readonly HttpClient _httpClient;
        private readonly string _baseUrl;
        private readonly string _xorKey;

        public DatabaseBackend(string baseUrl = "http://127.0.0.1:7720")
        {
            ...
            Logger.Info($"Database backend initialized for {_baseUrl}", "DatabaseBackend");
        }
        ...
        public async Task<bool> TestConnectionAsync()
        {
            try
            {
                ...
                Logger.Info("Database backend connection test successful", "DatabaseBackend");
                return true;
            }
            catch (Exception ex)
            {
                Logger.Error(ex, "Database backend connection test failed", "DatabaseBackend");
                return false;
            }
        }
        ...
        public async Task<T> ExecuteAsync<T>(DatabaseRequest request)
        {
            try
            {
                Logger.Debug($"Executing database operation: {request.Operation} on {request.Collection}", "DatabaseBackend");
                ...
                {
                    Logger.Error($"HTTP error: {response.StatusCode}", "DatabaseBackend");
                    return default(T);
                }
                ...
                {
                    Logger.Warning("Empty response from database backend", "DatabaseBackend");
                    return default(T);
                }
                ...
                Logger.Debug($"Database operation successful: {request.Operation}", "DatabaseBackend");
            ...
            {
                Logger.Error(ex, "Network error during database operation", "DatabaseBackend");
                return default(T);
            }
            catch (TaskCanceledException ex)
            {
                Logger.Error(ex, "Database operation timeout", "DatabaseBackend");
                return default(T);
            }
            catch (Exception ex)
            {
                Logger.Error(ex, "Database operation failed", "DatabaseBackend");
                return default(T);
            }
        }
        ...
        public async Task<UserInfo> AuthenticateUserAsync(string username, string password)
        {
            try
            {
                Logger.Info($"Authenticating user: {username}", "DatabaseBackend");
                ...
                {
                    Logger.Info($"User authentication successful: {username}", "DatabaseBackend");
                ...
                Logger.Warning($"User authentication failed: {username}", "DatabaseBackend");
                return new UserInfo { IsAuthenticated = false };
            }
            catch (Exception ex)
            {
                Logger.Error(ex, $"Authentication error for user: {username}", "DatabaseBackend");
                return new UserInfo { IsAuthenticated = false };
            }
        }
        ...
        public async Task<bool> RegisterUserAsync(string username, string email, string password)
        {
            try
            {
                Logger.Info($"Registering new user: {username}", "DatabaseBackend");
                ...
                if (result != null)
                {
                    Logger.Info($"User registration successful: {username}", "DatabaseBackend");
                    return true;
                }

                Logger.Warning($"User registration failed: {username}", "DatabaseBackend");
                return false;
            }
            catch (Exception ex)
            {
                Logger.Error(ex, $"Registration error for user: {username}", "DatabaseBackend");
                return false;
            }
        }
        ...
        public async Task<bool> UpdateUserLoginAsync(string userId)
        {
            ...
            {
                Logger.Error(ex, "Failed to update user login", "DatabaseBackend");
                return false;
            }
        }

        /// <summary>
        /// Log product usage analytics
        /// </summary>
        public async Task<bool> LogProductUsageAsync(string userId, string productName, string action, Dictionary<string, object> metadata = null)
        {
            try
            {
            ...
            {
                Logger.Error(ex, "Failed to log product usage", "DatabaseBackend");
                return false;
            }
        }
        ...
        private string GetXorKey()
        {
            ...
            {
                Logger.Error(ex, "Failed to decode XOR key", "DatabaseBackend");
                throw;
            }
        }
        ...
        private string XorEncrypt(string text, string key)
        {
            ...
            {
                Logger.Error(ex, "XOR encryption failed", "DatabaseBackend");
                throw;
            }
        }
        ...
        private string XorDecrypt(string hexText, string key)
        {
            ...
            {
                Logger.Error(ex, "XOR decryption failed", "DatabaseBackend");
                throw;
            }
        }
        ...
        public void Dispose()
        {
            ...
            {
                _httpClient?.Dispose();
                Logger.Info("Database backend disposed", "DatabaseBackend");
            }
            catch (Exception ex)
            {
                Logger.Error(ex, "Error disposing database backend", "DatabaseBackend");
            }
        }
    }
